...
exports.register = async (req, res) => {
  ...
  const existingUser = await UserModel.findOne({ name: req.body.name, email: req.body.email, phone: num });
  console.log("🚀 ~ exports.register= ~ existingUser:", existingUser)
  if (existingUser) return res.render("login", { data: { error: "Account already exists" }})

  if (req.body.register_repass !== req.body.password) return res.render("login", { data: { error: "Passwords don't match" }})
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (check === null) {
      return res.render("login", { data: {error: "Invalid email or password"} });
    }
